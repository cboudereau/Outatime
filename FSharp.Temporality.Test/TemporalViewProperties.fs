...
module ViewProperties = 
    [<Property>]
    let ``StartDate of view should be always greater or equal the given viewed startDate period`` period (temporaries : string Temporary seq) = 
    ...
    [<Property>]
    let ``EndDate of view should be always less or equal to the given viewed end date period`` () period (temporaries : string Temporary seq) = 
        let windowedTemporal = temporaries |> Temporality.clamp period
